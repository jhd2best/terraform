...
for s in 3 2 1; do
   if [ -d harmony_db_${s} ]; then
      ...
      rclone sync -P mainnet:pub.harmony.one/${FOLDER}/harmony_db_${shard} /root/harmony_db_${shard}
      break
   fi
done
...
rclone sync -P mainnet:pub.harmony.one/${FOLDER}/harmony_db_0 /root//harmony_db_0
